...
import { GetTopRatedShows } from '../APIfetcher/API';
import { useDispatch, useSelector } from 'react-redux';
import { getNewTopRatedShowPage } from '../redux/action/actions';
...
const TopRatedShow = () => {
    ...
    const getNewPage = (newPage) => {
        ...
        setPage(newPage);
        dispatch(getNewTopRatedShowPage(newPage))
    }

    const {isLoading, data} = GetTopRatedShows();  

    const topRatedShows = useSelector((state) => state.topRatedReducer.topRatedShows.results)
    ...
    <>
        ...
        <>
            <Flex p={'.5rem'} wrap={'wrap'} justify={'center'} gap={'2rem'}>
                {
                topRatedShows.map((currentElement) => {

...
import { GetTopRatedMovies } from '../APIfetcher/API';
import { useDispatch, useSelector } from 'react-redux';
import { getNewTopRatedMoviePage } from '../redux/action/actions';
...
const TopRatedMovie = () => {
    ...
    const getNewPage = (newPage) => {
        ...
        setPage(newPage);
        dispatch(getNewTopRatedMoviePage(newPage))
    }

    const {isLoading} = GetTopRatedMovies();  

    const topRatedMovies = useSelector((state) => state.topRatedReducer.topRatedMovies.results)
    ...
    <>
        ...
        <>
            <Flex p={'.5rem'} wrap={'wrap'} justify={'center'} gap={'2rem'}>
                {
                topRatedMovies.map((currentElement) => {
